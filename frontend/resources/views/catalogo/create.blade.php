<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-gray-800">Nuevo Libro</h1>
                <p class="text-sm text-gray-500 mt-0.5">Registrar un nuevo material bibliográfico</p>
            </div>
        ...
        <form method="POST" action="{{ route('catalogo.store') }}">
            ...
            <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="number" name="anioPublicacion" value="{{ old('anioPublicacion') }}" min="1500" max="2100"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-400"
                        placeholder="2024">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="ejemplaresTotales" value="{{ old('ejemplaresTotales', 1) }}" min="0"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-400">
                </div>
                ...
                <div>
                    ...
                    <select name="categoriaId"
                        ...
                        @foreach ($categorias as $categoria)
                            <option value="{{ $categoria['id'] }}" @selected(old('categoriaId') == $categoria['id'])>
                                {{ $categoria['nombre'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="editorialId"
                        ...
                        @foreach ($editoriales as $editorial)
                            <option value="{{ $editorial['id'] }}" @selected(old('editorialId') == $editorial['id'])>
                                {{ $editorial['nombre'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="sm:col-span-2">
                    ...
                    <select name="autorIds[]" multiple size="5"
                        ...
                        @foreach ($autores as $autor)
                            <option value="{{ $autor['id'] }}" @selected(in_array($autor['id'], old('autorIds', [])))>
                                {{ $autor['nombre'] }} {{ $autor['apellido'] }}
                            </option>
                        @endforeach
                    </select>
                    <p class="text-xs text-gray-400 mt-1">Mantén presionado Ctrl para seleccionar varios autores.</p>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="descripcion" rows="3"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-400"
                        placeholder="Descripción breve del libro">{{ old('descripcion') }}</textarea>
                </div>
            </div>
