...
| Código | Observación | Decisión | Verificación |
|--------|-------------|----------|--------------|
| OBS-23 | «Contraseñas de dev en 4 archivos» (el docente estimó 4; el trazado halló secretos en más ubicaciones). Desglose real: `docker-compose.yml` (defaults `postgres` y JWT dev), `backend/src/test/resources/application.yml` (secreto JWT hardcodeado y `test123`), `.github/workflows/ci.yml` (`test123`), migraciones Flyway `V3`/`V5`/`V7` (`admin123`/`biblio123`/`estudiante123` en claro), colección Postman, `README.md`, seeder y docs. El `.env` local con `Doctora2025` está en `.gitignore` y **no** se versiona. | **Resuelta (remediación ejecutada).** 1) `V7` re-hashea los semilla desde variables de entorno (`SEED_*`, placeholders de Flyway con guarda no-vacío); `V3`/`V5` sin referencias. 2) JWT de prueba y `test123` sustituidos por variables sin default (`TEST_JWT_SECRET`, `SPRING_DATASOURCE_PASSWORD`), alimentadas por el arnés. 3) CI ejecuta `scripts/run-tests.sh` (credenciales efímeras) y añade escaneo con gitleaks (`secret-scan`). 4) Docker Compose con `:?` sin defaults. 5) Postman parametrizado; README sin tabla. 6) Seeder BFF vaciado. Riesgo residual del histórico de Git documentado (no reescrito). | commit `da4df7a` (`fix(seguridad)`), `docs/seguridad/SECRETOS.md` (estado «Hecho»), `CHANGELOG.md` («Remediación de secretos OBS-23»), `.github/workflows/ci.yml` (trabajo `secret-scan`), `RUNBOOK.md §3.6` |
