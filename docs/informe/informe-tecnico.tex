...
informe documenta la arquitectura y sus decisiones (diez ADR), y sobre todo el
...
RQ2 & Determinar el nivel de seguridad alcanzado & 51 comprobaciones OWASP API Top 10; auditoría de SQL dinámico; SpotBugs con find-sec-bugs; ZAP en modo pasivo & Sí, parcialmente: el escaneo activo de ZAP sigue sin ejecutarse \\
RQ3 & Caracterizar el efecto de la caché & p95 de latencia del catálogo; hit ratio de Redis & Sí, con reservas (véase §\ref{sec:discusion}) \\
...
\midrule
\endhead
Adecuación funcional & \req{REQ-F-001}--\req{REQ-F-011} & 95 pruebas automatizadas; 9 de 11 requisitos funcionales verificados \\
...
Seguridad & \req{REQ-NF-002}, \req{REQ-NF-003}, \req{REQ-NF-006}, \req{REQ-NF-007}, \req{REQ-NF-009} & Auditoría OWASP de 51 comprobaciones, 51 superadas; auditoría de SQL dinámico con 0 hallazgos e instrumento autoprobado; SpotBugs con find-sec-bugs, un hallazgo de seguridad real (CSRF); ZAP pasivo: 0 alertas altas en API y frontend, 1 media en la API y 4 en el frontend. \textbf{Sin escaneo activo} \\
Mantenibilidad & \req{REQ-NF-005} & 95 pruebas, 0 fallos. Cobertura 42,51\,\% de línea y 16,84\,\% de rama, recalculable desde el crudo versionado. Por debajo del objetivo \\
...
se verificó sobre una respuesta real. Pero al examinarlo aparecieron dos
carencias que el hallazgo dejaba a la vista. La defensa 	extbf{no estaba documentada
en ningún ADR}, de modo que era una decisión que nadie había tomado
explícitamente; y 	extbf{ninguna prueba fallaba si alguien retiraba el atributo}, de
modo que toda la seguridad dependía de que nadie tocara esa línea. Una decisión
de seguridad que nadie escribió y que nadie vigila no es una decisión: es una
casualidad afortunada.

Ambas cosas están corregidas. El \adr{0010} registra la decisión con sus cuatro
alternativas y, sobre todo, con la razón de no haber elegido la más segura: el
coste de coordinar un token CSRF a través del BFF no se juzgó proporcionado para
este proyecto, y así se dice, en lugar de presentar la opción adoptada como si
fuera la mejor. Y \texttt{CsrfDefenseTest} vigila ahora el atributo en las dos
cookies, la de acceso y la de cierre de sesión --- esta última importa igual,
porque una cookie de borrado sin el atributo permitiría a un tercero forzar el
cierre de sesión de un usuario.

La prueba se validó por el procedimiento que este informe defiende en otros
puntos: se retiró temporalmente la línea \texttt{setAttribute("SameSite",
"Strict")} del código y se comprobó que dos de los cuatro casos fallaban. Una
prueba que no se ha visto fallar no prueba nada.

Queda declarada una limitación que no tiene arreglo desde el código: si un
navegador ignora \texttt{SameSite}, el sistema queda expuesto y el servidor no
puede detectarlo. Es el precio de apoyar la defensa en algo que ejecuta el
cliente.
...
(ii) el análisis dinámico con OWASP ZAP se ejecutó solo en modo pasivo: la araña recorrió la API sin autenticar y no hubo escaneo activo, de modo que la ausencia de alertas altas no evidencia ausencia de inyección;
(iii) la cobertura de rama es del 16,84\,\%, muy por debajo de lo deseable: en las tres mediciones se ha movido menos de punto y medio pese a pasar de 55 a 95 pruebas;
(iii bis) \req{REQ-U-002} no se cumple: Lighthouse da 78 en buenas prácticas
...
Los diez ADR están en \texttt{docs\-/adr\-/}, en formato MADR, cada uno con sus
