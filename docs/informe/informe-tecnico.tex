...
seguridad de 42 comprobaciones basada en OWASP API Security Top~10, la medición
...
RQ2 & Determinar el nivel de seguridad alcanzado & 42 comprobaciones OWASP API Top 10; auditoría de SQL dinámico & Sí, parcialmente: sin ZAP ni SpotBugs \\
RQ3 & Caracterizar el efecto de la caché & p95 de latencia del catálogo; hit ratio de Redis & Sí, en tres corridas \\
...
Eficiencia de desempeño & \req{REQ-R-001}--\req{REQ-R-004} & Medido parcialmente: p95 del catálogo en tres corridas k6. Login y préstamo sin medir \\
Compatibilidad & \req{REQ-NF-004} & Imágenes ancladas por digest SHA-256; cinco servicios en Docker Compose \\
Usabilidad & \req{REQ-U-001}, \req{REQ-U-002} & Lighthouse: 82/100/100/91 en una corrida móvil. SUS: \textbf{sin medir}, cero participantes \\
Fiabilidad & \req{REQ-NF-008}, \req{REQ-NF-010} & Tres defectos corregidos con prueba de regresión; 17 corridas de CI sin fallo \\
Seguridad & \req{REQ-NF-002}, \req{REQ-NF-003}, \req{REQ-NF-006}, \req{REQ-NF-007}, \req{REQ-NF-009} & Auditoría OWASP de 42 comprobaciones; auditoría de SQL dinámico con 0 hallazgos e instrumento autoprobado. \textbf{Sin ZAP} \\
Mantenibilidad & \req{REQ-NF-005} & Cobertura 38,85\,\% de línea y 16,31\,\% de rama, recalculable desde el crudo versionado. Por debajo del objetivo \\
...
\textbf{Respondida parcialmente.} La auditoría de 42 comprobaciones basada en
...
\texttt{@PreAuthorize("hasAnyRole('ADMIN', 'BIBLIOTECARIO')")}. La lección es
que una auditoría con cobertura parcial de rutas produce una falsa sensación de
seguridad, y es una amenaza a la validez que se recoge en §\ref{sec:validez}.
...
conservadas dan un p95 de 73,02, 64,89 y 26,41 milisegundos, con cero errores.
La corrida más rápida, con diferencia, fue la de \emph{caché fría} con la máquina
virtual Java ya caliente.
...
Esta conclusión es descriptiva, no causal: tres corridas sin intervalos de
confianza ni prueba inferencial no permiten afirmar una diferencia
estadísticamente significativa. Lo que sí permiten es \emph{no} afirmar la
mejora que se daba por supuesta, que es un resultado con valor propio. Contrastar
la hipótesis en serio exige repetir la medición con un acervo de decenas de miles
de títulos.
...
(ii) no se ejecutó un análisis dinámico de seguridad con OWASP ZAP;
(iii) la cobertura de rama es del 16,31\,\%, muy por debajo de lo deseable;
(iv) no se realizó inferencia estadística por insuficiencia de repeticiones;
